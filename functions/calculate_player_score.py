...
def calculate_player_score(player):
    print(player, player["_id"])
    # retrieve entries for calculation
    player_id = player["_id"]
    # put in list to make non consumable
    entries = list(entry_repository.get_entries_for_player(player_id))
...
def calculate_play_points(entries, player_id):
    ...
    for entry in entries:
        ...
        play_points_total += calculate_match_points(scores["player_score"], scores["opp_score"])
...
def calculate_attendance(entries):
    unique_dates = {
        (entry["datetime"].date() if isinstance(entry["datetime"], datetime)
